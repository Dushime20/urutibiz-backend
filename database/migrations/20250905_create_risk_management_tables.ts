...
export async function up(knex: Knex): Promise<void> {
  ...
  const hasRiskProfiles = await knex.schema.hasTable('product_risk_profiles');
  if (!hasRiskProfiles) {
  ...
  const hasRiskAssessments = await knex.schema.hasTable('risk_assessments');
  if (!hasRiskAssessments) {
    ...
    // Add foreign keys conditionally
    if (hasProducts && hasUsers && hasBookings) {
      await knex.schema.alterTable('risk_assessments', (table) => {
        table.foreign('product_id').references('id').inTable('products').onDelete('CASCADE');
  ...
  const hasComplianceChecks = await knex.schema.hasTable('compliance_checks');
  if (!hasComplianceChecks) {
    ...
    // Add foreign keys conditionally
    if (hasBookings && hasProducts && hasUsers) {
  ...
  const hasPolicyViolations = await knex.schema.hasTable('policy_violations');
  if (!hasPolicyViolations) {
    ...
    // Add foreign keys conditionally
    if (hasBookings && hasProducts && hasUsers) {
      await knex.schema.alterTable('policy_violations', (table) => {
        table.foreign('booking_id').references('id').inTable('bookings').onDelete('CASCADE');
        table.foreign('product_id').references('id').inTable('products').onDelete('CASCADE');
        table.foreign('renter_id').references('id').inTable('users').onDelete('CASCADE');
      });
  ...
  const hasEnforcementActions = await knex.schema.hasTable('enforcement_actions');
  if (!hasEnforcementActions) {
    ...
    // Add foreign keys conditionally
    if (hasBookings && hasProducts && hasUsers) {
      await knex.schema.alterTable('enforcement_actions', (table) => {
        table.foreign('booking_id').references('id').inTable('bookings').onDelete('CASCADE');
        table.foreign('product_id').references('id').inTable('products').onDelete('CASCADE');
        table.foreign('renter_id').references('id').inTable('users').onDelete('CASCADE');
      });
  ...
  const hasRiskConfigs = await knex.schema.hasTable('risk_management_configs');
  if (!hasRiskConfigs) {
    ...
    if (hasCategories && hasCountries) {
      await knex.schema.alterTable('risk_management_configs', (table) => {
        table.foreign('category_id').references('id').inTable('categories').onDelete('CASCADE');
