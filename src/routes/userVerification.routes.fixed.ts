 ...
 * Fixed TypeScript compatibility issues and improved structure
 */
...
// Type-safe wrapper for controllers that expect AuthenticatedRequest
const wrapController = (controller: any) => {
...
router.post('/submit-documents', 
  requireAuth, 
  cacheInvalidationMiddleware(['verification:*']), 
  wrapController(EnhancedUserVerificationController.submitDocuments)
);
...
router.get('/status', 
  requireAuth, 
  cacheMiddleware(verificationCacheOptions), 
  wrapController(EnhancedUserVerificationController.getVerificationStatus)
);
...
router.put('/resubmit', 
  requireAuth, 
  wrapController(UserVerificationController.resubmitVerification)
);
...
router.get('/documents', 
  requireAuth, 
  cacheMiddleware(verificationCacheOptions), 
  wrapController(UserVerificationController.getVerificationDocuments)
);
...
router.get('/history', 
  requireAuth, 
  cacheMiddleware(verificationCacheOptions), 
  wrapController(UserVerificationController.getVerificationHistory)
);
...
router.post('/user-verification', 
  requireAuth, 
  cacheInvalidationMiddleware(['verification:*']), 
  wrapController(UserVerificationController.submitVerification)
);

router.get('/user-verification', 
  requireAuth, 
  cacheMiddleware(verificationCacheOptions), 
  wrapController(UserVerificationController.getUserVerifications)
);

// Admin endpoints
router.post('/user-verification/review', 
  requireAuth, 
  requireAdmin, 
  cacheInvalidationMiddleware(['verification:*']), 
  wrapController(UserVerificationController.reviewVerification)
);
...
router.get('/ai-metrics', 
  requireAuth, 
  requireAdmin, 
  wrapController(EnhancedUserVerificationController.getAIProcessingMetrics)
);

router.post('/cancel/:verificationId', 
  requireAuth, 
  wrapController(EnhancedUserVerificationController.cancelAIProcessing)
);
