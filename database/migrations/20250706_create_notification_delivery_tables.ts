...
export async function up(knex: Knex): Promise<void> {
  // Check if notification_delivery_attempts table already exists
  const hasNotificationDeliveryAttempts = await knex.schema.hasTable('notification_delivery_attempts');
  if (!hasNotificationDeliveryAttempts) {
    await knex.schema.createTable('notification_delivery_attempts', (table) => {
      ...
      table.uuid('notification_id').notNullable().comment('Reference to notification');
    ...
    // Conditionally add FK if notifications table exists
    const hasNotifications = await knex.schema.hasTable('notifications');
    if (hasNotifications) {
      await knex.schema.alterTable('notification_delivery_attempts', (table) => {
        table.foreign('notification_id').references('notifications.id').onDelete('CASCADE');
      });
    }
  }

  // Check if notification_delivery_status table already exists
  const hasNotificationDeliveryStatus = await knex.schema.hasTable('notification_delivery_status');
  if (!hasNotificationDeliveryStatus) {
    ...
    const hasNotifications = await knex.schema.hasTable('notifications');
    if (hasNotifications) {
      await knex.schema.alterTable('notification_delivery_status', (table) => {
        table.foreign('notification_id').references('notifications.id').onDelete('CASCADE');
      });
    }
  }
  ...
  const hasUserNotificationPreferences = await knex.schema.hasTable('user_notification_preferences');
  if (!hasUserNotificationPreferences) {
    await knex.schema.createTable('user_notification_preferences', (table) => {
      table.uuid('user_id').primary().comment('Reference to user');
    ...
    // Conditionally add FK if users table exists
  ...
  console.log('✅ Notification delivery tables created successfully');
}
