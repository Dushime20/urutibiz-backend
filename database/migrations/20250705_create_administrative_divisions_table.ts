...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('administrative_divisions', (table) => {
    ...
    table.uuid('parent_id').nullable().references('id').inTable('administrative_divisions').onDelete('CASCADE');
  ...
  await knex.raw('CREATE INDEX idx_admin_divisions_coordinates ON administrative_divisions USING GIST (coordinates)');
  await knex.raw('CREATE INDEX idx_admin_divisions_bounds ON administrative_divisions USING GIST (bounds)');

  // Insert sample data for Rwanda (hierarchical structure)
  // Get Rwanda country ID first
  const rwandaCountry = await knex('countries').where('code', 'RW').first();
  
  if (rwandaCountry) {
    // Level 1: Provinces
    const provinces = await knex('administrative_divisions').insert([
      {
        ...
        population: 1132686,
        area_km2: 730.0,
        coordinates: knex.raw("ST_GeomFromText('POINT(30.0619 -1.9441)', 4326)")
      },
      {
        country_id: rwandaCountry.id,
        level: 1,
        ...
        population: 2595703,
        area_km2: 9458.0,
        coordinates: knex.raw("ST_GeomFromText('POINT(30.4383 -2.0000)', 4326)")
      },
      {
        country_id: rwandaCountry.id,
        level: 1,
        ...
        population: 1726370,
        area_km2: 3276.0,
        coordinates: knex.raw("ST_GeomFromText('POINT(29.6333 -1.6667)', 4326)")
      },
      {
        country_id: rwandaCountry.id,
        level: 1,
        ...
        population: 2589975,
        area_km2: 5963.0,
        coordinates: knex.raw("ST_GeomFromText('POINT(29.7500 -2.6000)', 4326)")
      },
      {
        country_id: rwandaCountry.id,
        level: 1,
        ...
        population: 2471239,
        area_km2: 5883.0,
        coordinates: knex.raw("ST_GeomFromText('POINT(29.2500 -2.2500)', 4326)")
      }
    ]).returning('*');

    // Level 2: Districts for Kigali City
    const kigaliProvince = provinces.find(p => p.code === 'KGL');
    if (kigaliProvince) {
      await knex('administrative_divisions').insert([
        {
          ...
          population: 530907,
          area_km2: 429.3,
          coordinates: knex.raw("ST_GeomFromText('POINT(30.1127 -1.9536)', 4326)")
        },
        {
          country_id: rwandaCountry.id,
          parent_id: kigaliProvince.id,
          ...
          population: 318061,
          area_km2: 166.7,
          coordinates: knex.raw("ST_GeomFromText('POINT(30.1000 -1.9667)', 4326)")
        },
        {
          country_id: rwandaCountry.id,
          parent_id: kigaliProvince.id,
          ...
          population: 284561,
          area_km2: 134.0,
          coordinates: knex.raw("ST_GeomFromText('POINT(30.0583 -1.9536)', 4326)")
        }
      ]);
    }
  }
    ...
    await knex('administrative_divisions').insert([
      {
        ...
        population: 4397073,
        area_km2: 696.0,
        coordinates: knex.raw("ST_GeomFromText('POINT(36.8219 -1.2921)', 4326)")
      },
      {
        country_id: kenyaCountry.id,
        level: 1,
        code: '001',
        name: 'Mombasa County',
        local_name: 'Mombasa County',
        type: 'county',
        population: 1208333,
        area_km2: 294.7,
        coordinates: knex.raw("ST_GeomFromText('POINT(39.6682 -4.0435)', 4326)")
      },
      {
        country_id: kenyaCountry.id,
        level: 1,
        code: '032',
        name: 'Kiambu County',
        local_name: 'Kiambu County',
        type: 'county',
        population: 2417735,
        area_km2: 2449.2,
        coordinates: knex.raw("ST_GeomFromText('POINT(36.8000 -1.0000)', 4326)")
      }
    ]);
  }
